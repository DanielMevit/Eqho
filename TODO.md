...
## Git Setup
- [ ] `git init` in `D:\Vibe Coding\Eqho`
- [ ] `git add .` and initial commit
- [ ] Create GitHub repo (e.g. `gh repo create DanielMevit/Eqho --private --source . --remote origin --push`)

## Optional
- [ ] To add to Windows startup: copy `dist\Eqho.exe` to `%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup` (build the .exe first with `build.ps1`).
